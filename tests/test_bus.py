...
from main.bus_logic import Bus, ROUTE #imports bus class and route data from core logic

def test_route_not_empty():
    assert len(ROUTE) > 2 #checks if the route has more than 2 coordinate points so the bus has a valid path to follow

def test_interpolation_midpoint(): 
    bus = Bus() #creates a new bust instance using the default route and speed
...
def test_route_loops():
    bus = Bus()
    bus.index = len(ROUTE) - 1
    bus.progress = 0.999

    bus.move(dt=5)  # move 5 seconds → guaranteed to cross boundary
